...
const stubFetchWithBlob = (mimeType: string, bytes: Uint8Array) => {
  vi.stubGlobal(
    'fetch',
    vi.fn(async () => new Response(bytes, { headers: { 'content-type': mimeType } })),
  )
}
...
describe('normalizeImageInput', () => {
  ...
  it('converts blob: URL to base64 with detected mime type', async () => {
    stubFetchWithBlob('image/png', new Uint8Array([1, 2, 3, 4]))
  ...
  it('converts data: URL to base64', async () => {
    stubFetchWithBlob('image/jpeg', new Uint8Array([9, 8, 7]))
...
describe('normalizeImageInputs', () => {
  ...
  it('processes mixed URLs in one batch', async () => {
    stubFetchWithBlob('image/png', new Uint8Array([1, 2]))
